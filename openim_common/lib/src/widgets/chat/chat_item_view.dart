...
class _ChatItemViewState extends State<ChatItemView> {
  ...
  bool _canRevoke(Message message) {
    // 检查消息发送时间是否在撤回时限内（2分钟内）
    final now = DateTime.now().millisecondsSinceEpoch;
    final sendTime = message.sendTime ?? 0;
    final canRevoke = (now - sendTime) < 2 * 60 * 1000; // 2分钟
    print('撤回检查: isISend=$_isISend, canRevoke=$canRevoke, sendTime=$sendTime, now=$now');
    return canRevoke;
  }

  void _showRevokeMenu() {
    ...
    return child = ChatItemContainer(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          onLongPress: () {
            print('长按消息 - 发送者ID: ${_message.sendID}');
            print('当前用户ID: ${OpenIM.iMManager.userID}');
            print('消息时间: ${_message.sendTime}');
